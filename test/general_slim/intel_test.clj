...
(deftest test-update-unit-intel
  ...
  (testing "main"
    (let [game-state {:red {:units {:a {:position [1 1] :id :a
      ...
      (is (= {:b {:side :red :id :b, :position [3 2], :age 4} ;; b is not in the current viewshed so is aged by 1
              :c {:side :blue :id :c, :position [3 3], :age 0} ;; c is in viewshed, so the position is updated to it's current position and age is 0
              :e {:side :blue :id :e, :position [5 5], :age 0} ;; e was not in the old intel set, so is brought in as new
