...
* FOV
* BUG: Units can't see themselves
* BUG: Can attack a unit you can't see

* Replace routing path with arrow, extend to attack with BANG! 
* Make unit HP fit on tiles
* Scenario text
...
* Casualties, recovery
* Better sprites
* Hover menu for units

== Scratch: Fields of view

Each unit has a field of view.

* A unit can see across an open plain or roads up to 5 sqaures away
* A Mountain blocks everything behind it, but a unit can see any units on the mountain itself
* Forests block everything behind it, but you can't see any units in the forest unless you are right next to it

Proposed algorithm:

* Get every location which is exactly 5 squares away from the unit.
* For each square on that radius, calculate the 
straight line 'path' to that square
* Walk that path, and return any square that they can "see" - i.e. stop when you get to a forest, or the square after a mountain
* Do this for every path
* Return the intersection of all points
