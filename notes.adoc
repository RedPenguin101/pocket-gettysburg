...
* BUG: end highlight / select on end turn
* Change order system to a sort of queue
** Issue move order, target square has a 'shadow' of unit on it but unit doesn't move
** Can issue attack order (or wait) from shadow. Attack order gets queued behind the move order
** Once attack/wait commmand is issued _then_ the unit moves and attacks
** Change attack menu to order menu: 'Order builder text': "Move to map reference X Y and attack Z"
